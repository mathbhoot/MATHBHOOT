...
if (root) {
      ...
      ? cards
          .map(({ card, collectionIndex }) => {
            ...
            const partDrawer = card.parts
              ? `<details class="part-drawer" open><summary>${escape(card.parts[activePartIndex]?.label)}</summary><div class="part-drawer-menu">${card.parts.map((part, partIndex) => `<button type="button" data-collection-index="${collectionIndex}" data-part-index="${partIndex}" class="${partIndex === activePartIndex ? "active" : ""}" aria-pressed="${partIndex === activePartIndex}">${escape(part.label)}</button>`).join("")}</div></details>`
              : "";
  ...
  const renderModule = () => {
    ...
    if (tabs)
      tabs.innerHTML = sessionItems
        .map(
          (related, itemIndex) =>
            `<button type="button" data-related-item="${itemIndex}" class="${itemIndex === state.itemIndex ? "active" : ""}" aria-pressed="${itemIndex === state.itemIndex}">${escape(related.title)}</button>`,
        )
        .join("");
    if (content)
      content.innerHTML = [
  ...
  const renderSidebar = () => {
    ...
    const sessionItems = getCollectionItems(
      selected.collection,
      state.partIndex,
    );
    if (related)
      related.innerHTML = sessionItems
        .map(
          (item, itemIndex) =>
            `<li class="${itemIndex === state.itemIndex ? "active" : ""}"><span>${escape(item.title)}</span><small>${escape(item.meta)}</small></li>`,
        )
        .join("");
    if (badges)
      badges.innerHTML = state.data.badges
        .map((badge) => `<span>${escape(badge)}</span>`)
        .join("");
  ...
  document.querySelector("#archiveGrid")?.addEventListener("click", (event) => {
    ...
    if (partButton) {
      selectSession(
        Number(partButton.dataset.collectionIndex),
        0,
        false,
        Number(partButton.dataset.partIndex),
      );
      return;
    }
    ...
    selectSession(
      ...
      true,
      Number(button.dataset.partIndex || 0),
    );
  });
