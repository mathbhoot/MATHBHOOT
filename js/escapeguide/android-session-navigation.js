...
if (escapePage && isAndroidDevice) {
  ...
  let navigationCopy = {
    ...
    announcement: "Current session opened",
  };
  let statusTimer;
  ...
  fetch("../../data/escapeguide/escape-guide-content.json")
    ...
    .then((data) => {
      if (data?.androidNavigation)
        navigationCopy = { ...navigationCopy, ...data.androidNavigation };
    })
    .catch(() => {
      /* Navigation remains usable with fallback interface text. */
    });
  ...
  escapePage.addEventListener("escape-guide:session-selected", (event) => {
    ...
    requestAnimationFrame(() =>
      sessionPreview.classList.add("session-arrival"),
    );
    window.setTimeout(
      () => sessionPreview.classList.remove("session-arrival"),
      1400,
    ...
    status.setAttribute(
      "aria-label",
      `${navigationCopy.announcement}${chapterTitle}`,
    );
