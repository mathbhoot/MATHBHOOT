(() => {
    ...
    const renderProfile = (data) => {
        ...
        const statusHeading = createElement('p', 'status-kicker', data.status.kicker);
        ...
        data.metrics.forEach((item) => {
            ...
            const progressValue = isError ? 100 : Math.min(100, Math.max(0, numericValue));
            fill.classList.add(`metric-fill-${progressValue}`);
            track.append(fill);
            metric.append(track, createElement('span', 'metric-value', isError ? item.value : `${numericValue}%`));
        ...
        anomaly.append(signal, createElement('p', 'anomaly-copy', data.anomaly.message));
        ...
        searchCopy.append(createElement('p', 'eyebrow', data.intruderSearch.kicker));
        const searchTitle = createElement('h2', 'intruder-title');
        searchTitle.id = 'intruderHeading';
        searchTitle.append(document.createTextNode(data.intruderSearch.titleLead + ' '));
        searchTitle.append(createElement('span', '', data.intruderSearch.titleAccent));
        ...
        data.intruderSearch.paragraphs.forEach((paragraph, index) => {
            narrative.append(createElement('p', index === data.intruderSearch.paragraphs.length - 1 ? 'intruder-vow' : '', paragraph));
        });
