(() => {
  ...
  const renderHero = (hero) => {
    ...
    order.append(
      make("p", "classified-stamp", "Restricted"),
      make("h2", "", hero.orderTitle),
      make("p", "", hero.orderText),
    );
  ...
  const renderMissions = (data) => {
    ...
    data.missions.forEach((mission) => {
      ...
      const missionLink = makeLink(
        data.missionActionLabel || "Create Now",
        "#application",
        "mission-link",
      );
  ...
  const buildField = (field) => {
    const group = make(
      "div",
      `application-field application-field-${field.type}`,
    );
  ...
  const bindDraftForm = (form, status) => {
    form.addEventListener("submit", async (event) => {
      ...
      draft.originalityConfirmed =
        formData.get("originalityConfirmed") === "on";
      try {
        store.saveDraft(draft);
      } catch {
        status.textContent =
          "This browser blocked local draft storage. Copy your text before leaving the page.";
        return;
      }

      if (!service || typeof service.submit !== "function") {
        status.textContent =
          "The secure submission service is temporarily unavailable. Your draft remains on this device.";
        return;
      }
      ...
      try {
        ...
        if (result.verificationSent) {
          status.textContent =
            "Verification email sent. Open its link, return here, and confirm once more to submit your idea.";
        } else {
          store.clearDraft();
          form.reset();
          status.textContent =
            "Submission received securely. Your reference is " +
            result.reference +
            ".";
        }
      } catch (error) {
        status.textContent =
          error?.message ||
          "Submission failed safely. Your draft remains on this device.";
      } finally {
  ...
  const renderApplication = (application) => {
    ...
    panel.append(
      heading,
      make("p", "application-intro", application.description),
      form,
    );
  ...
  const renderSidebar = (data) => {
    ...
    data.ranks.forEach((rank) => {
      ...
      item.append(
        make("span", "rank-mark", "◇"),
        make("strong", "", rank.name),
        make("small", "", rank.requirement),
      );
      ...
      closing.links.forEach((item) => {
        ...
        navigation.append(
          makeLink(
            item.label,
            item.href,
            "recruitment-action recruitment-action-secondary",
          ),
        );
      });
    }
  ...
  const bindMissionSelection = (workspace) => {
    ...
    if (!typeField) return;
    workspace
      .querySelectorAll(".mission-link[data-contribution-type]")
      .forEach((link) => {
        link.addEventListener("click", () => {
          const option = link.dataset.contributionType;
          if (
            option &&
            Array.from(typeField.options).some((item) => item.value === option)
          ) {
  ...
  const render = (data) => {
    const valid =
      ...
      Array.isArray(data.closing?.links);
    ...
    document.dispatchEvent(
      new CustomEvent("mathbhoot:recruitment-rendered", {
        detail: { workspace },
      }),
    );
    return true;
  ...
  fetch(source)
    .then((response) => {
      if (!response.ok)
        throw new Error(
          `Recruitment content request failed: ${response.status}`,
        );
    ...
    .catch(() => {
      app.replaceChildren(
        make(
          "p",
          "recruitment-error",
          "The recruitment channel is temporarily unavailable.",
        ),
      );
    });
})();
