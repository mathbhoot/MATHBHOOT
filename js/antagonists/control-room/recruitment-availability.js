...
const availabilityUrl =
  "../../../data/antagonists/control-room/recruitment-availability.json";
...
if (recruitmentPage && applicationHost) {
  ...
  const closeRecruitment = (workspace) => {
    ...
    try {
      sessionStorage.removeItem("mathbhoot.antagonistContribution.draft.v1");
    } catch {
      /* Storage is optional. */
    }
  };

  document.addEventListener("mathbhoot:recruitment-rendered", (event) =>
    closeRecruitment(event.detail?.workspace),
  );
  ...
  fetch(availabilityUrl, { credentials: "same-origin" })
    ...
    .catch(() => {
      availability = {
        enabled: false,
        eyebrow: "Recruitment channel secured",
        heading: "Applications are temporarily closed",
        description: "MATHBHOOT is not accepting submissions at this time.",
        notice: "No information can be submitted from this page.",
        actionLabel: "Applications Paused",
      };
      closeRecruitment(applicationHost.querySelector(".recruitment-workspace"));
    });
}
