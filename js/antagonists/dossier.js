(() => {
    ...
    if (!app || !viewer || !viewerImage || !viewerTitle || !viewerCode || !closeButton || !lockNotice) return;
    ...
    const showLockNotice = () => {
        ...
        showLockNotice.timeoutId = window.setTimeout(() => { lockNotice.hidden = true; }, 2600);
    };
    ...
    const render = (data) => {
        if (!data?.archive || !Array.isArray(data.categories) || !Array.isArray(data.dossiers) || !Array.isArray(data.navigation)) return false;
        ...
        if (data.archive.heroImage) {
            hero.style.setProperty('--archive-hero-image', `url("${data.archive.heroImage}")`);
        ...
        heroCopy.append(title, make('p', 'archive-description', data.archive.description));

        const recovery = make('aside', 'recovery-panel');
        recovery.setAttribute('aria-label', `Archive recovery ${data.archive.recovery} percent`);
        ...
        gauge.append(make('strong', '', `${data.archive.recovery}%`), make('small', '', 'Recovered'));
        recovery.append(gauge, make('span', 'recovery-update', 'Signal updated · live archive'));
        ...
        data.dossiers.forEach((record) => {
            const card = make('article', `dossier-card${record.locked ? ' is-locked' : ''}`);
            card.dataset.category = record.category;
            const cardTop = make('div', 'dossier-card-top');
            cardTop.append(make('span', 'dossier-code', record.id), make('span', 'dossier-status', record.status));
            ...
            const action = make('button', 'dossier-action', record.locked ? 'Locked · Coming Soon' : 'Open Dossier →');
            action.type = 'button';
            action.dataset.dossierId = record.id;
            action.setAttribute('aria-label', record.locked ? `${record.title} is locked and coming soon` : `Open ${record.title}`);
        ...
        warning.append(make('strong', '', 'Warning'), make('p', '', data.archive.warning));
    ...
    fetch('../../data/antagonists/restricted-dossiers.json')
        ...
        .then((data) => {
            if (!render(data)) throw new Error('Restricted archive data is incomplete.');
        })
        .catch(() => {
            app.innerHTML = '<p class="dossier-error">Restricted archive is temporarily offline.</p>';
        });
})();
