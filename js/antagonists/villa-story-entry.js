(() => {
  ...
  const renderEntry = (content) => {
    ...
    copy.append(
      title,
      createElement("p", "villa-story-feature__text", content.paragraph),
    );

    const action = createElement(
      "a",
      "profile-button villa-story-feature__action",
      content.actionLabel,
    );
  ...
  fetch(source)
    .then((response) => {
      if (!response.ok)
        throw new Error(`Villa story entry request failed: ${response.status}`);
